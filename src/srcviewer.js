...
(function () {
  ...
  function open(doc_id, source_id, ws_id, app) {
    ...
    function render() {
      ...
      const media = s ? Store.getMedia(s.mxc_uri) : null;
      ...
      // ---- prep blob/data URL ----
      const dataUrl = media && media.data_url;
      let blobUrl = null;
      if (dataUrl) {
        try {
          ...
          const blob = new Blob([arr], { type: s.mime || 'application/octet-stream' });
          blobUrl = URL.createObjectURL(blob);
        } catch (_) { blobUrl = dataUrl; }
      }
